...
def scaling(samples):
	...
	"""
	acum = 0
	# Transpose samples to scale by feature (column)
	...
	return numpy.asarray(samples).T.tolist()  # return to original
...
# convert samples so that all samples have same lenght as params a+bx1+cx2+ ... nxn..
for i in range(len(samples)):
...
while True:  # run gradient descent until local minima is reached
	...
	a = show_errors(params, samples, y)
	print(params)
	epochs = epochs + 1
	# if(oldparams == params or epochs == 5000):   #  local minima is found when there is no further improvement
	if(a < alfa):  # local minima is found when there is no further improvement
		print("\nTRAINING FINISHED")
		print("samples:")
		print(samples)
		print("final params:")
